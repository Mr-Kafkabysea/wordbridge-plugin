...
通过本地 MCP 服务连接正在运行的 Microsoft Word，读取当前测试文档及选区，并在用户确认后追加文字。

当前为 `0.1.0-dev` 开发验证版本，仅支持 Windows 桌面 Word 和项目测试目录内的普通正文场景。没有替换选区、文档枚举或自动撤销工具，也不支持网页版 Word、macOS 和 Linux。尚未发布安装包或建立完整客户端兼容性矩阵。
...
截至 2026-09-10，56 项自动化测试通过；本机普通正文的真实 MCP 确认追加成功，用户确认原生撤销可用。此次 MCP 撤销后没有独立读取正文摘要。主动取消确认的真实界面测试、复杂格式、多客户端并发和其他客户端兼容性仍待验证。
...
程序只连接已有 Word 实例，通过 `ActiveDocument` 读取名称、完整路径及 `ReadOnly`。期望文件必须是 `test-documents/` 内已存在的 `.docx`；程序不会打开文件、切换文档或读取正文。
...
- `invalid_test_file`、`not_available`、`com_error`、`path_error`：输入或检查失败，退出码为 1。
...
- 期望文件必须位于 `test-documents/` 内，并与 Word 当前文档匹配。只读、编辑保护、受保护视图、修订记录/修订开启、内容控件或文末表格等场景暂不支持。
- 试验阶段最多追加 10000 个 Python 字符，正文长度上限为 100000 个 Word 位置单位。预览会在内存读取测试文档正文以计算摘要，但不输出或保存原正文；待追加文字会显示在预览中。
